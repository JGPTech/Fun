#!/usr/bin/env python3
r"""
DHA PaiNN / SchNetPack decomposition probe v6
==========================================
...
Paper-ish first pass:
  ...
  python dha_painn_schnetpack_probe_v6.py --nframes 2500 --max-epochs 50 --analysis-frames 24 --response-frames 2 --device cuda --force-train
...
def run_interaction_analysis(model, R: np.ndarray, F_ref: np.ndarray, Z: np.ndarray, args: argparse.Namespace, run_dir: Path) -> Tuple[Dict[str, Any], List[Dict[str, Any]]]:
    ...
    for local_idx, frame_idx in enumerate(tqdm(frame_indices, desc="SchNetPack Fij analysis")):
        ...
        if local_idx < args.response_frames:
            ...
            ii, jj, _, _ = pair_geometry(R[frame_idx])
            Jflat = J[ii, jj]
            response_rows.append({
                "frame_idx": int(frame_idx),
                "topk_jaccard_attr_vs_response": jaccard(base_top, topk_indices(Jflat, args.topk_frac)),
                "spearman_attr_vs_response": spearman_np(base_scores, Jflat),
                "response_p99": float(np.nanpercentile(Jflat[np.isfinite(Jflat)], 99)) if np.isfinite(Jflat).any() else float("nan"),
            })
    ...
    if response_rows:
        summary["response_mean_topk_jaccard_attr_vs_response"] = float(np.nanmean([r["topk_jaccard_attr_vs_response"] for r in response_rows]))
        summary["response_mean_spearman_attr_vs_response"] = float(np.nanmean([r["spearman_attr_vs_response"] for r in response_rows]))
...
def make_plots(run_dir: Path, dist: np.ndarray, mag: np.ndarray, theta: np.ndarray, dev: np.ndarray, gauge_rows: List[Dict[str, Any]], response_rows: List[Dict[str, Any]]) -> None:
    ...
    if response_rows:
        ...
        vals = [r["topk_jaccard_attr_vs_response"] for r in response_rows]
        plt.bar(np.arange(len(vals)), vals)
        plt.xlabel("response frame")
        plt.ylabel("top-k Jaccard")
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="DHA SchNetPack PaiNN F_ij decomposition + gauge probe")
